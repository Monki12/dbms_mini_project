...
import { Shield, LayoutDashboard, Users, Calendar, DollarSign, LogOut } from 'lucide-react';
import { useAuthStore } from '../../store/useAuthStore';
import apiClient from '../../api/client';

const navItems = [
  { to: '/admin', label: 'Dashboard', icon: LayoutDashboard, end: true },
  { to: '/admin/patients', label: 'Patient Directory', icon: Users, end: false },
  { to: '/admin/appointments', label: 'Appointments', icon: Calendar, end: false },
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shadow-sm">
        <div className="h-16 flex items-center px-6 border-b border-slate-100 gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-primary-600 uppercase tracking-wider mt-0.5">Admin Portal</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50/50">
          <div className="flex items-center gap-3 py-1">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-semibold truncate text-slate-800">{user?.display_name || 'Admin'}</p>
              <p className="text-xs text-primary-600 font-bold uppercase tracking-wide mt-0.5">Administrator</p>
            </div>
            <button
